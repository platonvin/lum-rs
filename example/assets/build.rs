...
use block_mesh::{
    greedy_quads, GreedyQuadsBuffer, Voxel, VoxelVisibility, RIGHT_HANDED_Y_UP_CONFIG,
};
use lum::load_interface::{Metadata, SimpleFaceIndices, SimpleVertex};
use lum::types::{ivec3, Material};
use std::env;
use std::fs;
...
fn main() -> io::Result<()> {
    ...
    println!("cargo:rerun-if-changed=assets/palette.vox");
    ...
    let block_ids = process_asset_directory("blocks", &assets_dir, &out_dir, true)?;
    let model_ids = process_asset_directory("models", &assets_dir, &out_dir, false)?;

    generate_enums_file(&out_dir.join("asset_enums.rs"), &block_ids, &model_ids)?;
    println!(
...
fn generate_enums_file(path: &Path, blocks: &[String], models: &[String]) -> io::Result<()> {
    ...
    load_interface::{{
        cast_meta, cast_slice, BlockData, Metadata, ModelData, SimpleFaceIndices,
        SimpleVertex,
    }},
    types::Material
}};"
    )?;
    writeln!(file, "use vek::Vec3;")?;
...
// Generic function to generate an asset enum and its associated `load` implementation.
...
// --- Voxel Processing Logic ---

#[derive(Clone, Copy, Default, Eq, PartialEq)]
struct InternalVoxel(u8);
...
fn process_vox_file(
    ...
    assert!(
        scene.models.len() == 1,
        "VOX file must contain exactly one model (not VOX limitation)"
    );
...
fn process_palette(assets_dir: &Path, out_dir: &Path) -> io::Result<Option<PathBuf>> {
    ...
    if !source_path.exists() {
        ...
        // panic!("No assets/palette.vox found");
        return Ok(None);
    }
    ...
    // 1. Extract base colors and transparency from the main palette.
