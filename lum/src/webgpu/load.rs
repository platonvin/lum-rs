...
impl<'window> LoadInterface for InternalRendererWebGPU<'window> {
    ...
    fn load_mesh(&mut self, model: &ogt_vox::VoxModel, _make_vertices: bool) -> InternalMeshModel {
        let size = uvec3 {
            x: model.size_x,
            y: model.size_y,
            z: model.size_z,
    ...
    fn create_rayrace_voxel_image(
        ...
        let repacked_voxels = voxels.iter().map(|v| *v as Voxel).collect::<Vec<_>>();

        let data_u8 = unsafe {
            std::slice::from_raw_parts(repacked_voxels.as_ptr() as *const u8, buffer_size as usize)
        };
    ...
    fn free_mesh(&mut self, mesh: InternalMeshModel) {
        drop(mesh);
    }
    ...
    fn set_block_palette_mesh(&mut self, block_id: BlockId, triangles: FaceBuffers) {
    ...
    fn get_block_palette_mesh(&self, block_id: BlockId) -> &InternalMeshBlock {
    ...
    fn load_block_from_file(&mut self, block: BlockId, path: &str) {
        ...
        assert!(scene.models.len() == 1); // only one model per file supported for now
                                          // blocks are always BLOCK_SIZE x BLOCK_SIZE x BLOCK_SIZE
        let model = &scene.models[0];
        assert!(
            model.size_x == BLOCK_SIZE && model.size_y == BLOCK_SIZE && model.size_z == BLOCK_SIZE
        );
        self.load_block(block, model);
    }

    fn load_block(&mut self, block_id: BlockId, model: &ogt_vox::VoxModel) {
