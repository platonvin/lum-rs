...
use crate::{
    assert_assume,
    containers::{Arena, Array3D, BitArray3d},
    renderer::{
        ...
        types::{
            i16vec3, i8vec3, ivec3, ivec4, mat4, quat, u8vec3, u8vec4, uvec3, vec2, vec3, vec4,
            AoLut, IndexedVertices, InternalMeshFoliage, InternalMeshLiquid, InternalMeshModel,
        ...
        webgpu::{
            ...
            wal::{Image, Wal},
            MeshFoliageDesc, BLOCK_PALETTE_SIZE_X, BLOCK_PALETTE_SIZE_Y,
        },
        Settings,
    },
};
...
impl<'window> InternalRendererWebGPU<'window> {
    ...
    pub fn exec_copies(&mut self) {
        ...
        // at this point, current block_palette texture is
...
pub struct RendererWgpu<'window> {
    ...
    foliage_ques: Vec<Vec<FoliageRenderRequest>>,
            ...
            for zz in 0..dim_z {
                for yy in 0..dim_y {
                    for xx in 0..dim_x {
                        ...
                        padded_data[index] = self.renderer.current_world[(xx, yy, zz)];
                    }
                }
            }
...
impl<'window> RendererInterface for RendererWgpu<'window> {
    ...
    fn new(settings: &Settings, window: Window, foliages: &[MeshFoliageDesc]) -> Self {
        Self {
            renderer: unsafe { InternalRendererWebGPU::new(settings, window, foliages.to_vec()) },
            block_que: vec![],
            // mesh_que: vec![],
    ...
    fn draw_foliage(&mut self, foliage: &MeshFoliage, pos: &vec3) {
        ...
        if self.is_block_visible(*pos) {
            let corresponding_foliage_queue = &mut self.foliage_ques[foliage.0.stored_id as usize];
