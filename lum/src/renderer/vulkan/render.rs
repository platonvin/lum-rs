...
use crate::{
    ...
    renderer::{
        vulkan::{BLOCK_PALETTE_SIZE_X, BLOCK_PALETTE_SIZE_Y},
        *,
    },
};
...
impl InternalRendererVulkan {
    ...
    fn raygen_map_water(&mut self, _water: &InternalMeshLiquid<MatId>, pos: &vec3) {
        let command_buffer = self.cmdbufs.graphics_command_buffers.current();
        let quality_size = 32;
        #[repr(C)] // for push constants
        #[derive(AsU8Slice)] // allow cast to &[u8]
...
impl render_interface::FoliageDescriptionBuilder<MeshFoliageDescription, MeshFoliage>
...
impl RendererInterface for RendererVulkan<'_> {
    ...
    fn new(
        settings: &crate::renderer::Settings,
        window: Window,
