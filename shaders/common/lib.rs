...
pub use spirv_std::glam::*;
pub use spirv_std::glam::{bvec3, ivec3, vec3, Mat2};
pub use spirv_std::glam::{BVec3, FloatExt};
pub use spirv_std::image::SampledImage;
...
pub fn read_probe(probe_ipos: IVec3, radiance_cache: &Image!(3D, type=f32, sampled=false)) -> Vec3 {
...
pub fn sample_radiance_with_normal(
    position: Vec3,
    normal: Vec3,
...
pub fn load_norm(mat_norm: &Image!(subpass, type=u32, sampled=false)) -> Vec3 {
    let rgba = mat_norm.read_subpass(IVec2::ZERO);
    let gba = rgba.yzw().as_vec3();
    (gba / 255.0) * 2.0 - Vec3::splat(1.0)
}

pub fn load_mat(mat_norm: &Image!(subpass, type=u32, sampled=false)) -> i32 {
    mat_norm.read_subpass(IVec2::ZERO).x as i32
}

pub fn load_depth(depth_buffer: &Image!(subpass, type=f32, sampled=false)) -> f32 {
    depth_buffer.read_subpass(IVec2::ZERO).x * 1000.0
}

pub fn get_mat(voxel: i32, voxel_palette: &Image!(2D, type=f32, sampled=false)) -> Material {
...
pub fn get_origin_from_depth(depth: f32, clip_pos: Vec2, ubo: &UniformBufferObject) -> Vec3 {
    ubo.campos.xyz()
...
pub fn sample_lightmap_with_shift(
    pos: Vec2,
    xx: i32,
    yy: i32,
...
pub const COLOR_ENCODE_VALUE: f32 = 8.0;

pub fn decode_color(encoded_color: Vec3) -> Vec3 {
    encoded_color * COLOR_ENCODE_VALUE
}

pub fn encode_color(color: Vec3) -> Vec3 {
    color / COLOR_ENCODE_VALUE
}

pub fn get_block(blocks: &Image!(3D, type=i32, sampled=false), block_pos: IVec3) -> i32 {
    blocks.read(block_pos).x
}

pub fn voxel_in_palette(relative_voxel_pos: IVec3, block_id: i32) -> IVec3 {
...
pub fn voxel_in_bit_palette(relative_voxel_pos: IVec3, block_id: i32) -> IVec3 {
    let block_x = block_id % BLOCK_PALETTE_SIZE_X;
    let block_y = block_id / BLOCK_PALETTE_SIZE_X;
...
pub fn init_t_vals(ray_origin: Vec3, ray_direction: Vec3) -> (Vec3, Vec3, IVec3) {
...
pub fn bool_as_float(b: bool) -> f32 {
    if b {
        1.0
    } else {
        0.0
...
pub fn load_norm_glossy(mat_norm: &Image!(2D, type=u32, sampled=false), pixel: IVec2) -> Vec3 {
    let rgba = mat_norm.read(pixel);
    let gba = vec3(rgba.y as f32, rgba.z as f32, rgba.w as f32);
    (gba / 255.0) * 2.0 - Vec3::splat(1.0)
}

pub fn load_mat_glossy(mat_norm: &Image!(2D, type=u32, sampled=false), pixel: IVec2) -> i32 {
    mat_norm.read(pixel).x as i32
}

pub fn load_depth_glossy(depth_buffer: &Image!(2D, type=f32, sampled), pixel: IVec2) -> f32 {
    depth_buffer.fetch(pixel).x * 1000.0
}

pub fn rotate2d(a: f32) -> Mat2 {
    let s = a.sin();
    let c = a.cos();
    Mat2::from_cols(Vec2::new(c, s), Vec2::new(-s, c))
}

pub fn load_depth_sampled(
    depth_buffer: &SampledImage<Image!(2D, type=f32, sampled)>,
    uv: Vec2,
) -> f32 {
    depth_buffer.sample_by_lod(uv, 0.0).x * 1000.0
}

pub fn qtransform(q: Vec4, v: Vec3) -> Vec3 {
...
pub fn sample_radiance_with_normal2(
    radiance_cache: &Image!(3D, type=f32, sampled = false),
    position: Vec3,
    normal: Vec3,
) -> Vec3 {
...
pub fn sample_radiance_without_normal(
    radiance_cache: &Image!(3D, type=f32, sampled = false),
    position: Vec3,
) -> Vec3 {
    let mut total_weight = 0.0;
...
pub fn decode_depth(d: f32) -> f32 {
    d * 1000.0
}

pub fn load_depth_far(smoke_depth_far_subpass: &Image!(subpass, type=f32, sampled=false)) -> f32 {
    decode_depth(smoke_depth_far_subpass.read_subpass(IVec2::ZERO).x)
}

pub fn load_depth_near(smoke_depth_near_subpass: &Image!(subpass, type=f32, sampled=false)) -> f32 {
    decode_depth(smoke_depth_near_subpass.read_subpass(IVec2::ZERO).x)
}

pub fn rotate_vec2(v: Vec2, a: f32) -> Vec2 {
    let s = a.sin();
    let c = a.cos();
...
pub fn rotatem(a: f32) -> Mat2 {
    let s = a.sin();
    let c = a.cos();
    Mat2::from_cols(Vec2::new(c, s), Vec2::new(-s, c))
}

pub fn get_origin_from_depth2(ubo: &UniformBufferObject, depth: f32, clip_pos: Vec2) -> Vec3 {
    ubo.campos.xyz()
        + (ubo.horizline_scaled.xyz() * clip_pos.x)
        + (ubo.vertiline_scaled.xyz() * clip_pos.y)
        + (ubo.camdir.xyz() * depth)
